...
class BTDevice: NSObject {
    ...
    private var rangeChar: CBCharacteristic?
    private var patchChar: CBCharacteristic?
    ...
    var range: Int {
        ...
        set {
            ...
            _range = newValue
            if let char = rangeChar {
                peripheral.writeValue(Data(bytes: [UInt8(_range)]), for: char, type: .withResponse)
            }
        }
    }
    var patch: Int {
        ...
        set {
            guard _patch != newValue else { return }
            
            _patch = newValue
            if let char = patchChar {
                peripheral.writeValue(Data(bytes: [UInt8(_patch)]), for: char, type: .withResponse)
            }
        }
    }
    var name: String {
...
extension BTDevice: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        peripheral.services?.forEach {
            ...
            if $0.uuid == BTUUIDs.infoService {
                ...
                peripheral.discoverCharacteristics([BTUUIDs.blinkOn,BTUUIDs.blinkSpeed,BTUUIDs.range,BTUUIDs.patch], for: $0)
            } else {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        service.characteristics?.forEach {
            ...
            if $0.uuid == BTUUIDs.blinkOn {
                ...
                self.speedChar = $0
                peripheral.readValue(for: $0)
            } else if $0.uuid == BTUUIDs.range {
                self.rangeChar = $0
                peripheral.readValue(for: $0)
            } else if $0.uuid == BTUUIDs.patch {
                self.patchChar = $0
                peripheral.readValue(for: $0)
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.uuid == rangeChar?.uuid, let s = characteristic.value?.parseInt() {
            _range = Int(s)
            delegate?.deviceRangeChanged(value: _range)
        }
        if characteristic.uuid == patchChar?.uuid, let s = characteristic.value?.parseInt() {
            _patch = Int(s)
            delegate?.devicePatchChanged(value: _patch)
        }
